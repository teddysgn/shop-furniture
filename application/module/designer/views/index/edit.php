<?php
    ...
    $productInfo = $this->infoItem;

    $linkForm = URL::createLink('designer', 'index', 'form', array('product_id' => $productInfo['id']));

    $slbCategory		= Helper::cmsSelectbox('form[category_id]'  , 'inputbox form-control', $this->slbCategory   , $productInfo['category_id']  , 'width: 17.5rem; border: 1px solid black');
    $rowCategory        = Helper::cmsRowForm(''     , $slbCategory);

    $slbCollection		= Helper::cmsSelectbox('form[collection_id]'  , 'inputbox form-control', $this->slbCollection   , $productInfo['collection_id']  , 'width: 17.5rem;border: 1px solid black');
    $rowCollection        = Helper::cmsRowForm(''     , $slbCollection);



if($productInfo != null){
    if($productInfo['picture1'] != null){
        $path1          = 'cache' . DS . $productInfo['type'] . DS . $productInfo['name'] . DS . $productInfo['picture1'];
    } else {
        $path1          = 'product' . DS . $this->oldItem['name'] . DS . $this->oldItem['picture1'];
    }
    if($productInfo['picture2'] != null){
        $path2          = 'cache' . DS . $productInfo['type'] . DS . $productInfo['name'] . DS . $productInfo['picture2'];
    } else {
        $path2          = 'product' . DS . $this->oldItem['name'] . DS . $this->oldItem['picture2'];
    }
    if($productInfo['picture3'] != null){
        $path3          = 'cache' . DS . $productInfo['type'] . DS . $productInfo['name'] . DS . $productInfo['picture3'];
    } else {
        $path3          = 'product' . DS . $this->oldItem['name'] . DS . $this->oldItem['picture3'];
    }
    if($productInfo['picture4'] != null){
        $path4          = 'cache' . DS . $productInfo['type'] . DS . $productInfo['name'] . DS . $productInfo['picture4'];
    } else {
        $path4          = 'product' . DS . $this->oldItem['name'] . DS . $this->oldItem['picture4'];
    }
    if($productInfo['picture5'] != null){
        $path5          = 'cache' . DS . $productInfo['type'] . DS . $productInfo['name'] . DS . $productInfo['picture5'];
    } else {
        $path5          = 'product' . DS . $this->oldItem['name'] . DS . $this->oldItem['picture5'];
    }
    if($productInfo['picture6'] != null){
        $path6          = 'cache' . DS . $productInfo['type'] . DS . $productInfo['name'] . DS . $productInfo['picture6'];
    } else {
        $path6          = 'product' . DS . $this->oldItem['name'] . DS . $this->oldItem['picture6'];
    }
    if($productInfo['picture7'] != null){
        $path7          = 'cache' . DS . $productInfo['type'] . DS . $productInfo['name'] . DS . $productInfo['picture7'];
    } else {
        $path7          = 'product' . DS . $this->oldItem['name'] . DS . $this->oldItem['picture7'];
    }
    if($productInfo['picture8'] != null){
        $path8          = 'cache' . DS . $productInfo['type'] . DS . $productInfo['name'] . DS . $productInfo['picture8'];
    } else {
        $path8          = 'product' . DS . $this->oldItem['name'] . DS . $this->oldItem['picture8'];
    }
    if($productInfo['picture9'] != null){
        $path9          = 'cache' . DS . $productInfo['type'] . DS . $productInfo['name'] . DS . $productInfo['picture9'];
    } else {
        $path9          = 'product' . DS . $this->oldItem['name'] . DS . $this->oldItem['picture9'];
    }
    if($productInfo['picture10'] != null){
        $path10          = 'cache' . DS . $productInfo['type'] . DS . $productInfo['name'] . DS . $productInfo['picture10'];
    } else {
        $path10          = 'product' . DS . $this->oldItem['name'] . DS . $this->oldItem['picture10'];
    }
    if($productInfo['picture11'] != null){
        $path11          = 'cache' . DS . $productInfo['type'] . DS . $productInfo['name'] . DS . $productInfo['picture11'];
    } else {
        $path11          = 'product' . DS . $this->oldItem['name'] . DS . $this->oldItem['picture11'];
    }
    if($productInfo['picture12'] != null){
        $path12          = 'cache' . DS . $productInfo['type'] . DS . $productInfo['name'] . DS . $productInfo['picture12'];
    } else {
        $path12          = 'product' . DS . $this->oldItem['name'] . DS . $this->oldItem['picture12'];
    }

    $picture1Path	= UPLOAD_PATH . $path1;
    $picture2Path	= UPLOAD_PATH . $path2;
    $picture3Path	= UPLOAD_PATH . $path3;
    $picture4Path	= UPLOAD_PATH . $path4;
    $picture5Path	= UPLOAD_PATH . $path5;
    $picture6Path	= UPLOAD_PATH . $path6;
    $picture7Path	= UPLOAD_PATH . $path7;
    $picture8Path	= UPLOAD_PATH . $path8;
    $picture9Path	= UPLOAD_PATH . $path9;
    $picture10Path	= UPLOAD_PATH . $path10;
    $picture11Path	= UPLOAD_PATH . $path11;
    $picture12Path	= UPLOAD_PATH . $path12;

    if(is_file($picture1Path)==true){
        if(file_exists($picture1Path)==true) {
                $picture1Full = UPLOAD_URL . $path1;
    ...
    if(is_file($picture10Path)==true){
        if(file_exists($picture10Path)==true) {
            $picture10Full = UPLOAD_URL . $path10;
            $xhtml .= '<section class="product-category product">
                       <div class="product-wrapper" style="flex-flow: unset">
                                <a style="border-radius: unset; width: 100%; border: 1px solid black">
    ...
    if(is_file($picture11Path)==true){
        if(file_exists($picture11Path)==true) {
            $picture11Full = UPLOAD_URL . $path11;
            $xhtml .= '<section class="product-category product">
                       <div class="product-wrapper" style="flex-flow: unset">
                                <a style="border-radius: unset; width: 100%; border: 1px solid black">
    ...
    if(is_file($picture12Path)==true){
        if(file_exists($picture12Path)==true) {
            $picture12Full = UPLOAD_URL . $path12;
            $xhtml .= '<section class="product-category product">
                       <div class="product-wrapper" style="flex-flow: unset">
                                <a style="border-radius: unset; width: 100%; border: 1px solid black">
