...
class CollectionModel extends Model {
    private $_columns = array('id', 'name', 'created', 'created_by', 'modified', 'modified_by', 'status', 'ordering', 'description');
    private $_userInfo;
    public function __construct(){
        parent::__construct();

        $this->setTable(TBL_COLLECTION);
    ...
    public function changeStatus($arrParam, $option = null){
        if($option['task'] == 'change-ajax-status'){
            ...
            $result = array(
                'id'		=> $id,
                'status'	=> $status,
                'link'		=> URL::createLink('admin', 'collection', 'ajaxStatus', array('id' => $id, 'status' => $status))
            );
            return $result;
        }
    ...
    public function infoItem($arrParam, $option = null){
        if($option == null){
            $query[] = "SELECT `id`, `name`, `status`, `ordering`, `description` ";
            $query[] = "FROM `$this->table`";
