...
class Validate{
			...
			if($value['required'] == true && trim($this->source[$element])==null){
				...
				switch ($value['type']) {
					...
					case 'string':
						...
						break;
                    case 'null':
                        $this->validateNull($element, $value['options']['min'], $value['options']['max']);
                        break;
					case 'url':
    ...
    // Validate Integer
    private function validateCompare($element, $elementCompare){
    ...
    // Validate String
    private function validateNull($element, $min = 0, $max = 0){
        $length = strlen($this->source[$element]);
        if($length < $min) {
            $this->setError($element, 'is too short!');
        }elseif($length > $max){
	...
	private function validateStatus($element, $options){
		if(in_array($this->source[$element], $options['deny']) == true){
			$this->setError($element, 'Please choose another default value!');
		}
	}
	...
	private function validatePassword($element, $options){		
		if($options['action'] == 'add' || ($options['action'] == 'edit' && $this->source[$element] )){
			$pattern = '#^(?=.*\d)(?=.*[A-Z])(?=.*\W).{8,16}$#';	// Php1234!
			if(!preg_match($pattern, $this->source[$element])){
				$this->setError($element, 'is an invalid password. Password contains at least: '.'</br>&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp; '.'1 Uppercase '.'</br>&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp; '.'1 Lowercase '.'</br>&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp; '.'1 Number '.'</br>&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp; '.'1 Special Letter '.'</br>&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp; '.'8 - 16 characters in length');
			};
		}
	...
	private function validateFile($element, $options){
	    if ($this->source[$element]['name'] != null) {
            if(!filter_var($this->source[$element]['size'], FILTER_VALIDATE_INT, array("options"=>array("min_range"=>$options['min'],"max_range"=>$options['max'])))){
                $this->setError($element, 'File size does not match. The size is between .' . $options['min'] . ' and ' . $options['max']);
            }

            $ext = pathinfo($this->source[$element]['name'], PATHINFO_EXTENSION);
            if(in_array($ext, $options['extension']) == false){
                $this->setError($element, 'File extension does not match');
            }
        }
	}
}
